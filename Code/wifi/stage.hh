...
#define PRINT_DEBUG5(m,a,b,c,d,e)
  ...
  public:
    ...
    double Dottrace (const Pose& pose, const float scanWidth, const stg_ray_test_func_t func, Model *mod);
  ...
  */
  ...
  protected:
    ...
    Model* TestCollisionTree();
	 ...
	 double Dottrace(const Pose &pose, const float scanWidth, const stg_ray_test_func_t func);
  ...
  public:
	 ...
	 void SetLinefollowerReturn( int val );
  ...
  /// %ModelLinefollower class
  class ModelLinefollower : public Model
  {
  public:
	  class Sensor
	  {
	  public:
		  Pose pose; ///< The position of the sensors
		  Size size; ///< Doesn't have any significance
		  float scanWidth; ///< The width on the ground that will be scanned by this sensor
		  float value; ///< The color that is being seen by the sensor
	  };
  ...
  public:
	  ModelLinefollower( World* world, Model* parent,
			  const std::string& type );
	  virtual ~ModelLinefollower();
  ...
  private:
	  static Option showLinefollowerSensors;
	  static Option showLinefollowerRays;
  };
}; // end namespace stg
